...
j1Collider::j1Collider()
{
	...
	matrix[COLLIDER_LVL_END][COLLIDER_COIN] = false;
	...
	matrix[COLLIDER_ENEMY_BAT][COLLIDER_COIN] = false;
	...
	matrix[COLLIDER_ENEMY_SLIME][COLLIDER_COIN] = false;
	...
	matrix[COLLIDER_PLAYER_BASIC_SHOT][COLLIDER_COIN] = false;

	matrix[COLLIDER_COIN][COLLIDER_ENEMY_BAT] = false;
	matrix[COLLIDER_COIN][COLLIDER_ENEMY_SLIME] = false;
	matrix[COLLIDER_COIN][COLLIDER_PLAYER] = true;
	matrix[COLLIDER_COIN][COLLIDER_DEAD] = false;
	matrix[COLLIDER_COIN][COLLIDER_LVL_END] = false;
	matrix[COLLIDER_COIN][COLLIDER_GROUND] = false;
	matrix[COLLIDER_COIN][COLLIDER_PLAYER_BASIC_SHOT] = false;
...
void j1Collider::DebugDraw()
{
	...
	for (uint i = 0; i < MAX_COLLIDERS; ++i)
	{
		...
		case COLLIDER_COIN: // yellow
			App->render->DrawQuad(colliders[i]->rect, 255, 255, 53, alpha);
			break;
		default:
			break;
		}
